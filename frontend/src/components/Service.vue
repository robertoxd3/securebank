  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await serviceApi.put(`/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await serviceApi.post(null, this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await serviceApi.delete(`/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await serviceApi.get(null, {
