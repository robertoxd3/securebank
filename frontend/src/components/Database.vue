...
<template>
  ...
  <v-row v-if="dbFiltered.length > 0 && !loadingDatabases">
    <v-col cols="12" sm="6" md="6" v-for="(item, index) in dbFiltered" :key="index">
      <v-card :title="item.database">
        <v-card-text>
          <span>Nombre: <b>{{ item.database }}</b></span>
          <br />
          <span>Driver: <b>{{ item.driver }}</b></span>
          <br />
          <span>Host: <b>{{ item.host }}</b></span>
          <br />
          <span>Puerto: <b>{{ item.port }}</b></span>
          <br />

          <base-button title="Montar" type="primary" class="mt-3" @click="
            selectDatabase(item);
          dialogBd = true;
                        " />
        </v-card-text>
  ...
  <div class="text-center pt-3" v-if="dbFiltered.length == 0 && !loadingDatabases">
  ...
  <v-dialog v-model="dialogBd" width="800px" persistent>
    <v-card>
      ...
      <v-card-actions class="mx-auto">
        <base-button title="Montar" type="primary" :disabled="disableButtons" @click="
          mountDb();
        dialogBd = false;
                    ">
        </base-button>
